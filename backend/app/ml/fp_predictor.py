...
class FalsePositivePredictor:
    ...
    def _load_models(self):
        ...
        if not os.path.exists(CLASSIFIER_PATH):
            logger.warning(f"FP Classifier not found at {CLASSIFIER_PATH}. ML inference will be skipped.")
        ...
        try:
            ...
            logger.error(f"Failed to load ML Predictor: {e}. Inference will be skipped.")
            self.is_ready = False
        finally:
            self._models_loaded = True
    ...
    def adjust_scores(self, findings: list[dict]) -> list[float]:
        ...
        scores = [f.get("ml_score") if f.get("ml_score") is not None else 1.0 for f in findings]
        ...
        try:
            ...
            for i, f in enumerate(findings):
                ...
                logger.info(f"Rule: {f.get('rule_id')} | FP Probability: {fp_probability:.2f}")
